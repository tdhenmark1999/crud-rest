        ...
        <div class="panel-heading">Record Data
          <div class="tools dropdown">
            ...
            <ul role="menu" class="dropdown-menu pull-right">
              <li><a href="{{route('system.article.create')}}">Add new record</a></li>
            ...
            <tbody>
              ...
              <tr>
                <td class="cell-detail"> 
                  ...
                  <span class="cell-detail-description">{{"ID ".str_pad($article->id, 4, "0", STR_PAD_LEFT)}}</span>
                </td>
                <td class="cell-detail text-center"> 
                  <span><strong>{{Str::title($article->author?$article->author->name:NULL)}}</strong></span>
                </td>
                <td class="cell-detail">
                  <span>{{Helper::date_only($article->created_at)}}</span>
                  <span class="cell-detail-description">{{Helper::date_format($article->created_at,"h:i A")}}</span>
                </td>
                <td class="actions">
                    <div class="btn-group btn-hspace">
                      ...
                      <ul role="menu" class="dropdown-menu">
                        <li><a href="{{route('system.article.edit',[$article->id])}}">Edit Record</a></li>
                        <li class="divider"></li>
                        <li><a class="action-delete" href="#" data-url="{{route('system.article.destroy',[$article->id])}}" data-toggle="modal" data-target="#confirm-delete" title="Remove Record">Remove Record</a></li>
                      </ul>
                    </div>
                </td>
              </tr>
              @empty
              <td colspan="4" class="text-left"><i>No record found yet.</i> <a href="{{route('system.article.create')}}"><strong>Click here</strong></a> to create one.</td>
              @endforelse
            </tbody>
          </table>
